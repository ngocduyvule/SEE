...
if((isset($_POST["filenum"])) && (isset($_POST["filename"])) && (isset($_POST["fileagence"])) && (isset($_POST["fileagent"]))) {
	...
	$query = "UPDATE `see_files` SET `num`= '$filenum', `indice`= '$fileind', `agence`= '$fileagence', `agent`= '$fileagent', `affaire`= '$filename', `statut` = '$filestatut', `manque`= '$filemanque', `urgence`= '$fileurgence[0]', `dateenvoi`= NOW() WHERE `id` = '$fileid'";
	mysql_query("SET NAMES UTF8");
	$result = mysql_query($query) or die(mysql_error());
